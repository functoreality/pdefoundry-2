function [ux_interp, uy_interp] = static(domain_bounds, X, Y, Hmax, boundary_conditions, external_force, material_properties)
    % 创建PDE模型
    % profile on;
    model = createpde(2);
    points = [X(:), Y(:)];
    ...
    % 外力的插值函数
    Ffx = interp_2d(external_force(:,:,1));
    Ffy = interp_2d(external_force(:,:,2));
    ...
    % 设置方程系数
    specifyCoefficients(model, 'm', 0, 'd', 0, 'c', @elasticity_tensor, 'a', 0, 'f', @external_force_function);
    ...
    % 插值到指定的点
    ux_interp = interpolateSolution(result, points(:, 1), points(:, 2), 1);
    uy_interp = interpolateSolution(result, points(:, 1), points(:, 2), 2);
    ux_interp = reshape(ux_interp, size(X));
    uy_interp = reshape(uy_interp, size(Y));
    % profile viewer;
end