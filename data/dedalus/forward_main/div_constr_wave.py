...
from .multi_component_wave import MCompnWave2DPDE
try:
...
class DivConstraintWave2DPDE(MCompnWave2DPDE):
    r"""
    ...
    def reset_pde(self, rng: np.random.Generator) -> None:
        ...
        if c_obj.value[0] == 0. and c_obj.value[1] == 0.:
            c_obj.value[2] = 0.
        if self.valid_ic:
            c2_tmp = c_obj.value[2]
            c_obj.value[2] = 0.
            self.term_obj_dict["ut_ic/0"].div_constraint_2d_(
                rng, self.term_obj_dict["ut_ic/1"], c_obj)
            c_obj.value[2] = c2_tmp
    ...
    def get_dedalus_problem(self) -> Tuple:
        ...
        for i, ui_op in enumerate(u_list):
            ...
            # Damping term
            mu_i = self.term_obj_dict[f"mu/{i}"].gen_dedalus_ops(
                dist.Field(name=f"mu{i}", bases=self.bases))
        ...
        var_dict = {"u": u_list[0], "v": u_list[1]}  # , "p": p_op}
        return var_dict, problem


if __name__ == "__main__":
    my_args = get_cli_args(DivConstraintWave2DPDE)
    pde_data_obj = DivConstraintWave2DPDE(my_args)
