...
from ufl import (TrialFunction, TestFunction, grad, Measure, dot, nabla_div,
                 Identity, inner, sym)
...
class ElasticWaveEquation(elastic_wave.ElasticWaveEquation):
    __doc__ = "Generate dataset of the 2 components 2D elastic wave equation with FEniCSx." + \
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.time_sample_interval = args.time_sample_interval
    ...
    def get_cli_args_parser(cls) -> argparse.ArgumentParser:
        ...
        parser.add_argument("--time_sample_interval", type=int, default=1,
                            help="Sample interval in FEniCSx solver. Hint: this "
                            "option does not affect the output data shape. Total "
                            "time steps in solver is (n_t_grid - 1) "
    ...
    def solve_static(self) -> NDArray[float]:
        ...
        # shape [n_x_grid, n_y_grid, 2]
        if self.motion_type == self.IC_CHANGE:
            f_field = self.term_obj_dict["f_ic"].field
        else:
            f_field = self.term_obj_dict["f"].field
        ...
        if self.motion_type == self.BC_CHANGE:
            bc_dict = self.term_obj_dict["u_ibc"].get_dict_rep(
    ...
    def solve_dynamic(self) -> Tuple[NDArray[float], NDArray[float], NDArray[float]]:
        ...
        x_coord = self.coord_dict["x"]  # shape [n_x_grid, ]
        y_coord = self.coord_dict["y"]  # shape [n_y_grid, ]
        t_coord = self.coord_dict["t"]  # shape [n_t_grid, ]

        # shape [n_x_grid, n_y_grid, 2]
        u_ic_field = self.ic
        if self.motion_type == self.RANDOM_IVEL:
            ut_ic_field = self.term_obj_dict["ut_ic"].field
        else:
            ut_ic_field = np.zeros_like(u_ic_field)
        f_field = self.term_obj_dict["f"].field

        # shape [n_x_grid, n_y_grid, 2] and [n_t_grid, ]
        if self.motion_type == self.TIMEDEP_FORCE:
            fr_field = self.term_obj_dict["f_rt"].fr_field
            ft_field = self.term_obj_dict["f_rt"].ft_field
        else:
            fr_field = np.zeros((self.n_x_grid, self.n_y_grid, 2))
            ft_field = np.zeros(self.n_t_grid)

        u_ic = fem.Function(self.V)
        u_ic.interpolate(get_2d_interp_function(u_ic_field, x_coord, y_coord))
        ut_ic = fem.Function(self.V)
        ut_ic.interpolate(get_2d_interp_function(ut_ic_field, x_coord, y_coord))
        f = fem.Function(self.V)
        f.interpolate(get_2d_interp_function(f_field, x_coord, y_coord))
        fr = fem.Function(self.V)
        fr.interpolate(get_2d_interp_function(fr_field, x_coord, y_coord))
        ft_fn = get_1d_interp_function(ft_field, t_coord)

        property_func_dict = self.get_property_func_dict()
        G = property_func_dict["G"]
        lamb = property_func_dict["lamb"]
        rho = property_func_dict["rho"]
        ...
        u_sol = []
        u_sol_scat = []
        timesteps = (self.n_t_grid - 1) * self.time_sample_interval
        dt = self.STOP_SIM_TIME / timesteps

        # extrapolation to -1th time step
        un = u_ic  # 0th time step
        un_prev = fem.Function(self.V)  # -1th time step
        un_prev.x.array[:] = un.x.array[:] - dt * ut_ic.x.array[:]
        u_sol.append(self._eval(un))
        u_sol_scat.append(un_prev.x.array[:].reshape(-1, 2))
        ...
        dx = Measure("dx", domain=self.domain)
        ds = Measure("ds", domain=self.domain)
        ft = fem.Constant(self.domain, 0.0)

        for i in range(timesteps):
            ft.value = ft_fn(dt * i)
            u = TrialFunction(self.V)
            v = TestFunction(self.V)
            a = rho * dot(u, v) * dx + dt**2 * inner(sigma(u, lamb, G), epsilon(v)) * dx
            L = 2 * rho * dot(un, v) * dx - rho * dot(un_prev, v) * dx + \
                dt**2 * dot(f, v) * dx + dt**2 * ft * dot(fr, v) * dx + \
                dt**2 * dot(T, v) * ds
            petsc_options = {"ksp_type": "preonly", "pc_type": "lu"}
            problem = LinearProblem(a, L, bcs=bcs, petsc_options=petsc_options)
            uh = problem.solve()
            ...
            if (i + 1) % self.time_sample_interval == 0:
                u_sol.append(self._eval(un))
                u_sol_scat.append(un.x.array[:].reshape(-1, 2))
        u = np.stack(u_sol, axis=2)  # shape [n_x_grid, n_y_grid, n_t_grid, 2]
        u_scat = np.stack(u_sol_scat, axis=1)  # shape [n_nodes, n_t_grid, 2]
    ...
    def sigma(u: fem.Function,
        ...
        material properties.
        """
        epsilon = ElasticWaveEquation.epsilon
    ...
    def get_property_func_dict(self) -> Dict[str, fem.Function]:
        ...
        rho_field = self.term_obj_dict["rho"].field  # density
        ...
        rho = fem.Function(self.W)
        rho.interpolate(get_2d_interp_function(rho_field, x_coord, y_coord))
        return {"G": G, "lamb": lamb, "rho": rho}
